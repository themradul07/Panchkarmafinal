...
interface Therapy {
  ...
  benefits: string[];
  duration: string;
  recommendedSessions: string;  // New
  color: string;
}
      ...
      <DialogContent className="max-w-3xl p-0 overflow-hidden">
        ...
        <div className="p-6 space-y-6">
          ...
          <div className="flex items-center space-x-4">
            ...
            <Badge variant="outline" className="text-sm">{therapy.duration}</Badge>
            <Badge variant="secondary" className="text-sm">{therapy.recommendedSessions}</Badge>
          </div>

          {/* Description */}
          <div>
            <h3 className="text-lg font-semibold text-foreground mb-2">About This Therapy</h3>
            <p className="text-muted-foreground leading-relaxed text-base">{therapy.description}</p>
          </div>

          {/* Indications */}
          {therapy.indications && therapy.indications.length > 0 && (
            <div>
              <h3 className="text-lg font-semibold text-foreground mb-2">Indications / Uses</h3>
          ...
          {therapy.precautions && therapy.precautions.length > 0 && (
            <div>
              <h3 className="text-lg font-semibold text-foreground mb-2">Precautions / Contraindications</h3>
              <ul className="list-disc list-inside text-muted-foreground space-y-1">
          ...
          {/* Procedure */}
          {therapy.procedure && therapy.procedure.length > 0 && (
            <div>
              <h3 className="text-lg font-semibold text-foreground mb-2">Procedure</h3>
